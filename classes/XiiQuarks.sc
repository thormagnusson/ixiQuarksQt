...
/*
comment_on_ixiquarks: OS X 10.6.8
ixiQuarks version 7 (-Unofficial Build-)
Thanks for this software - I use it for real time experimental electronic improvisation. Some remarks after first use are:

1. amplifier does not route audio after a preset load and change of mode to on unless the out bus(es) goes to the audio card outputs.
2. wavescope after a preset load initialize with a fixed width and number of channels (2).
3. livecoder increases and not decreases a code's volume with key [.
4. soundscratcher volume has the same behavior as above in number 3.
5. grainbox volume cannot be "selected" to increase or decrease with keys [].
	...
	*new {
	...
	initXiiQuarks {
		...
		*/
		...
		win = Window.new(name, Rect(point.x, point.y, 275, 224), resizable:false);
